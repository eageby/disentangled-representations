...
def representation_variance(model, data, subset=None):
    ...
    for batch in progress:
        var.append(
                tf.math.reduce_variance(model.encode(batch['image'])[0], axis=0)
                )
...
def intact_dimensions_kld(model, data,subset=None, tolerance=1e-2):
    ...
    for batch in progress:
        ...
        kld.append(
                tf.reduce_mean(
                    dist.Gaussian.kld(dist.Gaussian, mean, log_var, 0., 0.)
                    , axis=0)
                )
...
def metric(model, dataset, training_votes=800, subset=1000):
    ...
    samples = []
    progress = disentangled.utils.TrainingProgress(dataset.take(training_votes), total=training_votes)
    ...
    accuracy = np.sum(factors == estimated_factors) / tf.size(factors, out_type=tf.int64)
    return accuracy
