...
def betavae(
    ...
    log_likelihood = tf.reduce_mean(
        tf.reduce_sum(output_dist.log_likelihood(target, x_mean, x_log_var), axis=1),
        axis=0,
    )
    ...
    objective.add_metric(-log_likelihood, aggregation="mean", name="-loglikelihood")
...
def factorvae(
    ...
    kld_discriminator = tf.reduce_mean(
        tf.math.log(
            discriminator_probability / ((1 - discriminator_probability) + tolerance)
        )
    )
...
def betatcvae(
    ...
    log_qzx = tf.reduce_sum(prior_dist.log_likelihood(z, z_mean, z_log_var), axis=1)
    ...
    log_qz = tf.reduce_logsumexp(tf.reduce_sum(log_qz_matrix, axis=2), axis=1) - log_nm
