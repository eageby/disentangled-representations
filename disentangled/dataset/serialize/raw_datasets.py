...
class DSprites:
    ...
    @staticmethod
    @gin.configurable(module="disentangled.dataset.serialize.raw_datasets.DSprites")
    def get_serialized_path():
        return disentangled.utils.get_data_path() / "serialized" / "DSprites.tfrecords"

    @staticmethod
    def get_file_path():
        file_path = disentangled.utils.get_data_path() / "downloads/DSprites.h5"
        file_path.parent.mkdir(exist_ok=True, parents=True)
        return file_path
    ...
    def get_index(factors):
        ...
        for factor, _ in reversed(list(enumerate(DSprites.factors))):
            indices += factors[factor] * base
            base *= DSprites.num_values_per_factor[factor]
    ...
    def batch_indices(batch_size):
        random_state = disentangled.utils.get_numpy_random_state(gin.REQUIRED)
        factors = [
            random_state.choice(DSprites.num_values_per_factor[f], batch_size)
            for f, _ in enumerate(DSprites.factors)
        ]

        fixed_factor = random_state.choice(len(DSprites.factors))
        fixed_factor_value = random_state.choice(
            DSprites.num_values_per_factor[fixed_factor]
        )
        factors[fixed_factor] = fixed_factor_value

        return DSprites.get_index(factors), fixed_factor, fixed_factor_value

    @staticmethod
    def generator(batch_size):
        for _ in itertools.count():
            idx, factor, value = DSprites.batch_indices(batch_size)
            idx = np.sort(idx)
            yield idx, factor, value

    @staticmethod
    def read():
        data = h5py.File(DSprites.path, "r", swmr=True)["imgs"]
        ...
        def wrapper(idx, factor, value):
            def inner(idx, factor, value):
                ...
                for i in idx:
                    im.append(np.expand_dims(data[i], axis=-1))
                im = np.stack(im)
    ...
    @gin.configurable(module="disentangled.dataset.serialize.raw_datasets.DSprites")
    def create(batch_size, num_parallel_calls=tf.data.experimental.AUTOTUNE):
        if DSprites.path is None:
            DSprites.path = tf.keras.utils.get_file(
                str(DSprites.get_file_path().resolve()),
                "https://github.com/deepmind/dsprites-dataset/raw/master/dsprites_ndarray_co1sh3sc6or40x32y32_64x64.hdf5",
            )

        def dict_map(image, factor, value):
            image.set_shape((None, 64, 64, 1))
            factor.set_shape(())
            value.set_shape(())
        ...
        dataset = index.map(DSprites.read())
    ...
    @staticmethod
    def set_shapes(element):
        element["image"].set_shape((None, 64, 64, 1))
        element["factor"].set_shape(())
        element["factor_value"].set_shape(())
